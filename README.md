...
```
  ...
  请选择要执行的操作：
    ...
    7) 查看登录信息  显示面板登录地址、用户名、密码（需 root）

  请输入 1 - 7 后回车（直接回车 = 1 安装正式版）:
```

- 选 3 时再提示输入版本号（直接输数字即可，例如 `3.1.1`，会自动补 `v`）
- 选 4 / 5 / 6 / 7 执行完就退出，不会继续安装；**选 6 会先要你输入 `yes` 确认**，其他内容视为取消
- 5) 用户名和密码都可以改，**直接回车 = 该项不改**（用户名 3-32 位字母/数字/下划线，密码 ≥8 位，都输两遍确认）
- 7) 需要 root；显示面板地址（含反代域名，若已配）、用户名、密码（见下方「凭据记录」）
...
**凭据记录（忘记密码时怎么找回）：** 安装/改凭据时会把用户名和密码记一份到
`/etc/fwpanel/credentials.json`（**权限 0600，仅 root 可读**），用菜单 `7) 查看登录信息` 就能查回地址/用户名/密码；
面板登录校验用的仍是 pbkdf2 哈希、不是这份明文。不需要这个便利可以随时删除该文件
（面板功能不受影响；删掉后忘记密码就只能用菜单 5 设一个新的）。
