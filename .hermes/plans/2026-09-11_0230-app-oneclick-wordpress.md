# 应用一键部署实施计划 — fwpanel2 v3.1.0 ✅ 已完成（2026-09-11）

> **状态**：已实现并发布 v3.1.0 beta（tag v3.1.0 / commit 0d4f2b0 / release 386463047），
> 三台机器（VM-115 / sg1 / sg2）已升级；后端 255 例全过、VM-115 真机全流程通过、真浏览器 UI 回归 22/22。
> 与计划的偏差：① 模板清单按用户要求为 WordPress/Typecho/Nextcloud/Vaultwarden 四个；
> ② 反向代理+证书+上传限制三项底座在实现中一并落地；③ 追加了「重试部署」与失败原因展示。
